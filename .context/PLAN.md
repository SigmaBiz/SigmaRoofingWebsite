...
- [x] **✅ COMMIT (2026-06-16):** all uncommitted work → commit `a2ab5f8`. (The first draft PR went to the WRONG repo `sigma-roofing-calendly` — closed + branch deleted.)
- [x] **✅ DEPLOY REPO RECONCILED + ARCHIVE/CLEAN SPLIT (2026-06-16):** Railway prod = **`origin` = `SigmaBiz/SigmaRoofingWebsite`**, branch was `staging-mobile-fix` (Antonio switching → `main`). **`origin/design-reskin` = full ARCHIVE** (`a2ab5f8` — tr3 engine, 22 dev pages, takeoff R&D, .context docs). **`launch` branch = CLEAN production** (pruned the 22 dev pages from App.tsx + deleted the files). Build VERIFIED green (dev chunks gone, /estimate intact, home 3D-free).
- [ ] **SHIP IT** — **Antonio:** (1) switch Railway "Branch connected to production" `staging-mobile-fix → main`; (2) review + **merge the `launch → main` PR**; Railway auto-deploys oksigma.com (Node server — SEO/sitemap/robots are server-side). Railway env vars (`GOOGLE_API_KEY`, both Turnstile keys, `ANTHROPIC_API_KEY`, SendGrid/Calendly) — **NOT `TURNSTILE_DEV_BYPASS`**.
